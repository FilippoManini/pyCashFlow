...
class Transaction(Base):
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            ...
            set(self.file_paths or []) == set(other.file_paths or [])
        )
...
class DBManager:
    ...
    def insert(self, t: Transaction) -> bool:
        with self.Session() as session:
            ...
            try:
                ...
                print(f"Insert failed due to exception: {e}")
                return False
    ...
    def delete(self, id_to_delete: int) -> bool:
        with self.Session() as session:
            ...
            if result.rowcount > 0:
                session.commit()
                print(f"Transaction with id={id_to_delete} deleted.")
                return True
            else:
                print(f"No transaction found with id={id_to_delete}.")
                session.rollback() #lo fa ugualmente
                return False
    ...
    def update(self, id: int, 
                ...
                file_paths: list[str]|None = None) -> bool:
        with self.Session() as session:
            t = session.get(Transaction, id)
            print(f"\n{t}: Transaction to update")
            if t is None:
                print(f"No transaction found with id={id}.")
                return False  
            
            stmt = db.update(Transaction).where(Transaction.id == id).values(
            ...
            result = session.execute(stmt)
            if result.rowcount == 0:
                print(f"No transaction found with id={id} to update.")
                return False
            session.commit()
    ...
    def select(self, 
        ...
        with self.Session() as session:
            ...
            # if file_paths:
            #     conditions.append(Transaction.file_paths == file_paths)
