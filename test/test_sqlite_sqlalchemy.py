...
class TestBaseDBManager(unittest.TestCase):
    ...
    def test_insert_transaction(self):
        t = Transaction(name="Insert", 
                        amount=20.0, 
                        category="Cat", 
                        date=datetime.date.today(), 
                        file_paths=["home/coop2.pdf",
                                     "taxes/coop1.pdf"])
        result = self.db_manager.insert(t)
        self.assertTrue(result)
    ...
    def test_insert_missing_name(self):
        ...
        t = Transaction(name=None, amount=10.0, category="TestCat", date=datetime.date.today(), file_paths=[])
        result = self.db_manager.insert(t)
        self.assertFalse(result)

    def test_insert_missing_amount(self):
        # Amount is required, should fail
        t = Transaction(name="Test", amount=None, category="TestCat", date=datetime.date.today(), file_paths=[])
        result = self.db_manager.insert(t)
        self.assertFalse(result)

    def test_insert_missing_category(self):
        # Category is required, should fail
        t = Transaction(name="Test", amount=10.0, category=None, date=datetime.date.today(), file_paths=[])
        result = self.db_manager.insert(t)
        self.assertFalse(result)

    def test_insert_missing_date(self):
        # Date is required, should fail
        t = Transaction(name="Test", amount=10.0, category="TestCat", date=None, file_paths=[])
        result = self.db_manager.insert(t)
        self.assertFalse(result)

    def test_insert_invalid_amount_type(self):
        # Amount should be a float, not a string
        t = Transaction(name="Test", amount="not_a_float", category="TestCat", date=datetime.date.today(), file_paths=[])
        result = self.db_manager.insert(t)
        self.assertFalse(result)
    ...
    def test_insert_invalid_file_paths_type(self):
        ...
        with self.assertRaises(ValueError):
            t = Transaction(name="Test", amount=10.0, category="TestCat", date=datetime.date.today(), file_paths="not_a_list")

    def test_delete_transaction(self):
        t = Transaction(name="ToDelete", 
                        amount=40.0, 
                        category="Cat", 
                        date=datetime.date.today(), 
                        file_paths=[])
        self.db_manager.insert(t)
        deleted = self.db_manager.delete(1)  # Assuming the ID of the inserted transaction is 1
        self.assertTrue(deleted)

    def test_update_transaction(self):
        t = Transaction(name="T1", 
                        amount=30.0, 
                        category="Cat", 
                        date=datetime.date.today(), 
                        file_paths=[])
        self.db_manager.insert(t)
        updated = self.db_manager.update(1, 
                                         name="Updated",
                                         date=datetime.date(2025, 1, 1),)
        self.assertTrue(updated)

    def test_select_by_id(self):
        t = Transaction(name="ById", 
                        amount=60.0, 
                        category="Cat", 
                        date=datetime.date.today(), 
                        file_paths=[])
    ...
    def test_search_transaction_found(self):
        t1 = Transaction(name="Search1", 
                         amount=50.0, 
                         category="Cat1", 
                         date=datetime.date.today(), 
                         file_paths=[])
        t2 = Transaction(name="Search2", 
                         amount=70.0, 
                         category="Cat2", 
                         date=datetime.date.today(), 
...
class TestRealDBManager(unittest.TestCase):
    ...
    def test_insert(self):
        """simulazione di 500 transazioni annue per 10 anni
        per questo test le performance non sono un problema
        """
        # with self.db_manager.Session() as session:
        #     session.query(Transaction).delete()
        #     session.commit()
        ...
        test_category = ["home", "taxes", "utilities", "groceries", "entertainment", "transportation", "healthcare", "education", "clothing", "miscellaneous"]

        with self.db_manager.Session() as session:
            for i in range(5000):
                t = Transaction(name=f"Transazione {i}", 
    ...
    def test_select_1(self):
        results = self.db_manager.select(name="Transazione 1")
        self.assertEqual(len(results), 1)

    def test_select_2500(self):
        results = self.db_manager.select(name="Transazione 2500")
        self.assertEqual(len(results), 1)

    def test_select_4999(self):
        results = self.db_manager.select(name="Transazione 4999")
        self.assertEqual(len(results), 1)
    
    def test_select_category_home(self):
        results = self.db_manager.select(category="home")
        print(f"Found {len(results)} transactions in the 'home' category.")
...
class TestBigDBManager(unittest.TestCase):
    ...
    def test_select_random(self):
        results = self.db_manager.select(name=f"Transazione {random.randint(1, 100000)}")
        print(results)
        self.assertEqual(len(results), 1)