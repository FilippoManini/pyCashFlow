...
class TestFileManager(unittest.TestCase):
    ...
    def test_save_custom_file(self):
        ...
        result = self.file_manager.save_custom_file(
            self.test_file, date, self.test_category
        )
    ...
    def test_save_custom_file_invalid_path(self):
        ...
        result = self.file_manager.save_custom_file(
            "nonexistent.txt", "2025-01-01", self.test_category
        )
    ...
    def test_update_file(self):
        ...
        result = self.file_manager.update_file(
            old_paths, new_paths, date, self.test_category
        )
    ...
    def test_update_file_date(self):
        ...
        original_path = self.file_manager.save_custom_file(
            self.test_file, old_date, self.test_category
        )
        self.assertIsNotNone(original_path)
        if original_path:
            result = self.file_manager.update_file_date([original_path], new_date)

            # self.assertGreater(len(result), 0)
            self.assertTrue(
                all(os.path.basename(p).startswith(new_date) for p in result)
            )
